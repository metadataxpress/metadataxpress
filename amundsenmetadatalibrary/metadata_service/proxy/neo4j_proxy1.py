...
from amundsenmetadatalibrary.metadata_service.entity.popular_table import PopularTable
from amundsenmetadatalibrary.metadata_service.entity.table_detail import Application, Column, Reader, Source, \
    Statistics, Table, Tag, User, Watermark
from amundsenmetadatalibrary.metadata_service.entity.tag_detail import TagDetail
from amundsenmetadatalibrary.metadata_service.entity.user_detail import User as UserEntity
from amundsenmetadatalibrary.metadata_service.exception import NotFoundException
from amundsenmetadatalibrary.metadata_service.proxy.base_proxy import BaseProxy
from amundsenmetadatalibrary.metadata_service.proxy.statsd_utilities import timer_with_counter
from amundsenmetadatalibrary.metadata_service.util import UserResourceRel
...
class Neo4jProxy(BaseProxy):
    ...
    def _get_user_table_relationship_clause(relation_type: UserResourceRel, tbl_key: str = None,
        ...
        if relation_type == UserResourceRel.follow:
            relation = '(usr{user_matcher})-[rel:FOLLOW]->(tbl{tbl_matcher})'
        elif relation_type == UserResourceRel.own:
            relation = '(usr{user_matcher})<-[rel:OWNER]-(tbl{tbl_matcher})'
        elif relation_type == UserResourceRel.read:
            relation = '(usr{user_matcher})-[rel:READ]->(tbl{tbl_matcher})'
        else:
            raise NotImplementedError('The relation type {relation_type} is not defined!')
        return relation
    ...
    def get_table_by_user_relation(self, *, user_email: str, relation_type: UserResourceRel) -> Dict[str, Any]:
        ...
        :return:
        """
        rel_clause = self._get_user_table_relationship_clause(relation_type=relation_type,
                                                                   tbl_key='',
                                                                   user_key=user_email)
        query = textwrap.dedent("""
    ...
    def add_table_relation_by_user(self, *,
        ...
        user_email_clause = 'key: "{user_email}"'
        tbl_key = 'key: "{table_uri}"'
