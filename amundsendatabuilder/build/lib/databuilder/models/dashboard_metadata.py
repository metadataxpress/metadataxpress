...
class DashboardMetadata(Neo4jCsvSerializable):
    ...
    DASHBOARD_name_NODE_LABEL = 'Lastreloadtime'
    DASHBOARD_name = 'value'
    DASHBOARD_name_FORMAT = '{dashboard_group}://{dashboard_name}/_lastreloadtime'
    DASHBOARD_name_RELATION_TYPE = 'name'
    name_DASHBOARD_RELATION_TYPE = 'name_OF'
    ...
    def _get_dashboard_name_key(self):
        # type: () -> str
        return DashboardMetadata.DASHBOARD_name_FORMAT.format(dashboard_group=self.dashboard_group,
                                                                          dashboard_name=self.dashboard_name)
    ...
    def _create_next_node(self):
        ...
        if self.name:
            yield {NODE_LABEL: DashboardMetadata.DASHBOARD_name_NODE_LABEL,
                   NODE_KEY: self._get_dashboard_name_key(),
                   DashboardMetadata.DASHBOARD_name: self.name}
    ...
    def _create_next_relation(self):
        ...
        if self.name:
            yield {
                RELATION_START_LABEL: DashboardMetadata.DASHBOARD_NODE_LABEL,
                RELATION_END_LABEL: DashboardMetadata.DASHBOARD_name_NODE_LABEL,
                RELATION_START_KEY: self._get_dashboard_key(),
                RELATION_END_KEY: self._get_dashboard_name_key(),
                RELATION_TYPE: DashboardMetadata.DASHBOARD_name_RELATION_TYPE,
                RELATION_REVERSE_TYPE: DashboardMetadata.name_DASHBOARD_RELATION_TYPE
            }
